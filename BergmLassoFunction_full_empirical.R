...
BergmLasso_full_empirical<- function(formula, 
                       data = NULL,
                       ...
                       delta0=2)
{
 ...
 #regression results by bayesian addpative lasso
  ...
  message(" > BERGM Lasso full empirical MCMC start")
  ...
  for (j in 1:tot.iters) {
    ...
    for (h in 1:nchains) {
      ...
      if (j %% 20 == 0) {
        low <- j - 19
        high <- j
        lambda2 <-  2/ (colMeans(1/(invTau2Samples[low:high, ,h ])))
      }
  ...
  out = list(formula = formula, specs = specs,lambda2=lambda2Samples,
